...
import {
  ...
  splitBill,
  payBill,
} from '@api/client/services/bills'
...
export const useCreateBill = (tableId: TableId) => {
  ...
  return useMutation((billData: CreateBillRequest) => createBill(tableId, billData), {
    onSuccess: () => {
...
export const useDeleteBill = (tableId: TableId) => {
  const queryClient = useQueryClient()
  return useMutation((billId: BillId) => deleteBill(tableId, billId), {
    onSuccess: () => {
      queryClient.invalidateQueries(['tables', tableId])
    },
  })
...
export const useSplitBill = (tableId: TableId) => {
  const queryClient = useQueryClient()
  return useMutation(
    ({ billId, splitData }: { billId: BillId; splitData: SplitBillRequest }) =>
      splitBill(tableId, billId, splitData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['tables', tableId])
      },
...
export const usePayBill = (tableId: TableId) => {
  const queryClient = useQueryClient()
  return useMutation((billId: BillId) => payBill(tableId, billId), {
    onSuccess: () => {
      queryClient.invalidateQueries(['tables', tableId])
    },
  })
