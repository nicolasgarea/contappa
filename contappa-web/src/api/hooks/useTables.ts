...
import {
  ...
  updateTable,
  deleteTable,
} from '@api/client/services/tables'
...
export const useCreateTable = () => {
  ...
  return useMutation((tableData: CreateTableRequest) => createTable(tableData), {
    onSuccess: () => {
...
export const useDeleteTable = () => {
  const queryClient = useQueryClient()
  return useMutation((tableId: TableId) => deleteTable(tableId), {
    onSuccess: () => {
      queryClient.invalidateQueries(['tables'])
    },
  })
